...
const {  state, saveCreds } =await useMultiFileAuthState(`./session`)
   ...
   // source code https://github.com/WhiskeySockets/Baileys/blob/master/Example/example.ts#L61
        ...
        if (!pn('+' + phoneNumber).isValid()) {
            console.log(chalk.red('Invalid phone number. Please enter your full international number (e.g., 255792021944 for Tanzania, 254784517274 for Kenya, etc.) without + or spaces.'));
            process.exit(1);
        }
...
try{
   ...
   await conn.groupAcceptInvite('LfTFxkUQ1H7Eg2D0vR3n6g');          
...
console.log(color('DAVE-MD Connected ✅','blue'))
                }
    ...
    if (global.statusview){
        try {
            ...
            if (mek.key && mek.key.remoteJid === 'status@broadcast') {
                // Just mark as read, no reactions
